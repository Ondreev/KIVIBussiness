...
document.addEventListener('sheets-ready', () => {
  // Берём уже распарсенные данные из общего загрузчика
  const dataRows = window.DATASETS.data;    // лист "Данные"
  const costRows = window.DATASETS.ebitda;  // лист "ebitda"
  ...
  // Строки текущего месяца с валидными значениями
  const validRows = dataRows.filter(r =>
    r["Дата"]?.startsWith(ym) && clean(r["ТО"]) > 0 && clean(r["расчет ASP"]) > 0
  );

  // Средние метрики за текущий месяц
  ...
  // Считаем расходы из листа ebitda
  let totalCosts = 0;
  ...
  // Текст советника (логика сохранена)
