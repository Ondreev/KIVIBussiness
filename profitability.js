...
document.addEventListener('sheets-ready', () => {
  ...
  const costs = window.DATASETS.ebitda;  // "ebitda"
  ...
  const validRows = data.filter(r => r["Дата"]?.startsWith(ym) && clean(r["ТО"]) > 0 && clean(r["расчет ASP"]) > 0);
  ...
  const rendered = costs.map(row => {
    ...
    totalCosts += cost;
    return { name: row["Статья"], value: cost, label: type.includes("%") ? `${row["Статья"]} (${value}%)` : row["Статья"] };
  }).filter(Boolean);
  ...
  rendered.forEach(r => { block.innerHTML += `<div style='display:flex;justify-content:space-between;margin-bottom:4px;'><span>– ${r.label}</span><span>${format(r.value)}</span></div>`; });
