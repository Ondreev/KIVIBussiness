// oracle.js — прогноз по тайм-слотам с галочками (основано на твоём файле)
...
document.addEventListener("sheets-ready", () => {
  ...
  // строки текущего месяца: до вчера и за сегодня
  const thisMonthRows = data.filter(r => {
    if (!isSameMonth(val(r, COLS.date), Y, M)) return false;
    const p = parseYMD(val(r, COLS.date)); return p && p.d < D && clean(val(r, COLS.revenue)) > 0;
  });
  ...
  // средние по месяцу
  const avgTo = Math.round(thisMonthRows.reduce((s,r)=>s+clean(val(r,COLS.revenue)),0) / (thisMonthRows.length || 1));
  const avgTr = Math.round(thisMonthRows.reduce((s,r)=>s+clean(val(r,COLS.traffic)),0) / (thisMonthRows.length || 1));
  ...
  // пересоздаём блок при повторной инициализации
  ...
  function renderOracle() {
    ...
    const factTo = todayRows.reduce((s,r)=>s+clean(val(r,COLS.revenue)),0);

    let cumTo = 0, cumTr = 0;
    for (const [period, share] of Object.entries(dayPercents)) {
